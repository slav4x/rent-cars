"use client";

import { useEffect, useRef, useState } from "react";

import styles from "./PanelDashboardSelect.module.sass";

export type PanelDashboardSelectOption = {
    value: string;
    label: string;
};

type PanelDashboardSelectProps = {
    name?: string;
    options: readonly PanelDashboardSelectOption[];
    defaultValue?: string;
    value?: string;
    onChange?: (value: string) => void;
};

export default function PanelDashboardSelect({
    name,
    options,
    defaultValue,
    value,
    onChange,
}: PanelDashboardSelectProps) {
    const initialValue = defaultValue ?? options[0]?.value ?? "";
    const [innerValue, setInnerValue] = useState(initialValue);
    const [width, setWidth] = useState(0);
    const measureRef = useRef<HTMLSpanElement>(null);
    const currentValue = value ?? innerValue;

    const selectedLabel =
        options.find((option) => option.value === currentValue)?.label ??
        options[0]?.label ??
        "";

    useEffect(() => {
        if (!measureRef.current) {
            return;
        }

        setWidth(measureRef.current.offsetWidth + 22);
    }, [selectedLabel]);

    return (
        <label className={styles.select}>
            <select
                name={name}
                value={currentValue}
                onChange={(event) => {
                    const nextValue = event.target.value;

                    setInnerValue(nextValue);
                    onChange?.(nextValue);
                }}
                className={styles.input}
                style={{ width: width ? `${width}px` : undefined }}
            >
                {options.map((option) => (
                    <option key={option.value} value={option.value}>
                        {option.label}
                    </option>
                ))}
            </select>

            <span ref={measureRef} className={styles.measure}>
                {selectedLabel}
            </span>

            <span className={styles.icon} aria-hidden="true">
                <svg
                    width="10"
                    height="5"
                    viewBox="0 0 10 5"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                >
                    <path
                        d="M9.79231 0.206184C9.72592 0.140851 9.64693 0.0889952 9.55989 0.0536073C9.47286 0.0182193 9.37951 0 9.28523 0C9.19094 0 9.09759 0.0182193 9.01056 0.0536073C8.92353 0.0889952 8.84454 0.140851 8.77814 0.206184L5.50708 3.39864C5.44069 3.46397 5.3617 3.51583 5.27467 3.55122C5.18763 3.58661 5.09428 3.60483 5 3.60483C4.90572 3.60483 4.81237 3.58661 4.72533 3.55122C4.6383 3.51583 4.55931 3.46397 4.49291 3.39864L1.22186 0.206184C1.15546 0.140851 1.07647 0.0889952 0.989437 0.0536073C0.902405 0.0182193 0.809054 0 0.714771 0C0.620487 0 0.527137 0.0182193 0.440104 0.0536073C0.353072 0.0889952 0.27408 0.140851 0.207685 0.206184C0.0746641 0.336784 0 0.513451 0 0.697599C0 0.881748 0.0746641 1.05841 0.207685 1.18901L3.48589 4.38844C3.88763 4.78004 4.4322 5 5 5C5.56779 5 6.11237 4.78004 6.51411 4.38844L9.79231 1.18901C9.92533 1.05841 10 0.881748 10 0.697599C10 0.513451 9.92533 0.336784 9.79231 0.206184Z"
                        fill="#C9A77F"
                    />
                </svg>
            </span>
        </label>
    );
}
